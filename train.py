...
def main():
    ...
    transform = transforms.Compose([transforms.ToPILImage(), transforms.Resize(
        (config.INPUT_IMAGE_HEIGHT, config.INPUT_IMAGE_WIDTH)), transforms.ToTensor()])

    # Create the train and test datasets
    train_datasets = SegmentationDataset(
        image_paths=train_images, mask_paths=train_masks, transforms=transform)
    test_datasets = SegmentationDataset(
        image_paths=test_images, mask_paths=test_masks, transforms=transform)

    print(
        f'[INFO]: Found {len(train_datasets)} examples in the training set...')
    ...
    test_loader = DataLoader(test_datasets, shuffle=True, batch_size=config.BATCH_SIZE,
                             pin_memory=config.PIN_MEMORY, num_workers=os.cpu_count())
